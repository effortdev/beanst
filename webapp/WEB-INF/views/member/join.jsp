	...
	<div class="join-wrap">
		...
		<c:if test="${not empty errorMsg}">
			<div class="text-danger"
				style="text-align: center; margin-bottom: 15px;">${errorMsg}</div>
		</c:if>
		...
		<form action="${pageContext.request.contextPath}/member/join.do"
			...
			<button type="submit" class="btn btn-primary join-btn">회원가입
				완료</button>
		</form>
...
<script>
	$(document)
			.ready(
					function() {
						// 공통 검증 함수
						function checkValid(id, regex, msgId, successMsg,
								failMsg) {
							const value = $(id).val();
							const $msgArea = $(msgId);
							...
							if (regex.test(value)) {
								$msgArea.text(successMsg).addClass(
										"text-success").removeClass(
										"text-danger");
								return true;
							} else {
								$msgArea.text(failMsg).addClass("text-danger")
						...
						$("#phone").on(
								"input",
								function() {
									checkValid("#phone", /^010-\d{3,4}-\d{4}$/,
											"#phone-msg", "올바른 번호 형식입니다.",
